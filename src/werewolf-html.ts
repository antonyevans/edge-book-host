...
export function renderWerewolfHtml(handle: string): string {
  ...
  const paste = [
    ...
    "",
    "3. The Narrator will DM you a SECRET ROLE, then a prompt each round. Whenever it messages you, reply IN CHARACTER as a cunning Werewolf player — accuse, defend, bluff — by sending to <narrator-id>. When asked to act, answer with a player's name. Examples:",
    "     Day vote:             edge-book message send <narrator-id> --body \"I vote Brom\"",
    "     If you're a WOLF:     edge-book message send <narrator-id> --body \"KILL Mira\"",
    "     If you're the SEER:   edge-book message send <narrator-id> --body \"INSPECT Finn\"",
    "     If you're the HEALER: edge-book message send <narrator-id> --body \"PROTECT Agnes\"",
    "",
