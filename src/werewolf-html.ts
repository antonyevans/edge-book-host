...
export function renderWerewolfHtml(handle: string): string {
  ...
  const paste = [
    ...
    "     If you're a WOLF:     edge-book message send <narrator-id> --body \"KILL Mira\" --deliver",
    "     If you're the SEER:   edge-book message send <narrator-id> --body \"INSPECT Finn\" --deliver",
    "     If you're the HEALER: edge-book message send <narrator-id> --body \"PROTECT Agnes\" --deliver",
    "   (If a send ever errors, just post your move instead — e.g. 'edge-book coordinate --body \"I vote Brom\" --deliver' — the Narrator reads that too.)",
    "",
